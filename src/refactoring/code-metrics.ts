...
export async function analyzeFile(
  ...
  try {
    ...
    if (linesOfCode > config.maxFileLines) {
      ...
      refactoringReasons.push(`File is too large (${linesOfCode} lines, recommended max: ${config.maxFileLines})`);
    }

    if (imports > config.maxDependencies) {
      needsRefactoring = true;
      refactoringReasons.push(`Too many dependencies (${imports}, recommended max: ${config.maxDependencies})`);
    }
...
export async function analyzeProject(
  ...
  async function scanDirectory(dirPath: string) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (!['node_modules', 'dist', '.git', '.vite', 'out'].includes(entry.name)) {
  ...
  const filesNeedingRefactoring = fileMetrics.filter(m => m.needsRefactoring).length;
  ...
  const refactoringOpportunities: RefactoringOpportunity[] = fileMetrics
    ...
    .flatMap(m => 
      m.refactoringReasons.map(reason => ({
        ...
        severity: m.linesOfCode > config.maxFileLines * 2 ? 'high' as const : 'medium' as const,
        filePath: m.filePath,
        description: reason,
  ...
  return {
    ...
    totalLinesOfCode,
    averageFileSize: fileMetrics.length > 0 ? totalLinesOfCode / fileMetrics.length : 0,
...
export function generateRefactoringReport(metrics: ProjectMetrics): string {
  ...
  if (metrics.refactoringOpportunities.length === 0) {
    report.push('No refactoring opportunities identified. Code quality looks good!');
  } else {
    const grouped = metrics.refactoringOpportunities.reduce((acc, opp) => {
    ...
    for (const severity of ['high', 'medium', 'low'] as const) {
      ...
      if (opportunities.length > 0) {
        report.push(`### ${severity.toUpperCase()} Priority (${opportunities.length})`);
        report.push('');
