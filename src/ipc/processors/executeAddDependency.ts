...
export async function executeAddDependency({
  ...
  try {
    ...
    command = await generateCommandWithFallbacks(appPath, "addDependency", {
      packages,
    });
  } catch {
    // Fallback to npm command if detection fails
    command = `npm install --legacy-peer-deps ${packageStr}`;
  }
