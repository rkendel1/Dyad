...
describe("Console Component", () => {
  ...
  it("renders console with toolbar and messages", () => {
    ...
    expect(
      screen.getByText("Application running on port 3000"),
    ).toBeInTheDocument();
  });
  ...
  it("filters messages correctly", () => {
    ...
    expect(screen.getByText("Error: Module not found")).toBeInTheDocument();
    expect(
      screen.queryByText("Starting application..."),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Application running on port 3000"),
    ).not.toBeInTheDocument();
  });
  ...
  it("enhances error detection with pattern matching", () => {
    ...
    expect(
      screen.queryByText("Successfully connected to database"),
    ).not.toBeInTheDocument();
  });
  ...
  it("includes keyboard shortcut hints in button titles", () => {
    ...
    expect(
      screen.getByTitle(/Copy all logs \(Ctrl\+Shift\+C\)/),
    ).toBeInTheDocument();
    expect(
      screen.getByTitle(/Export logs to file \(Ctrl\+Shift\+S\)/),
    ).toBeInTheDocument();
  ...
  it("includes CLI input component", () => {
    ...
    expect(
      screen.getByPlaceholderText(/Enter command or app input.../),
    ).toBeInTheDocument();
  });
