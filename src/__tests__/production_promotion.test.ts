...
describe('Production Promotion', () => {
  ...
  describe('updateEnvironmentVariables', () => {
    it('should create new environment file with production variables', async () => {
      ...
      expect(envLocalCall[1]).toContain('SUPABASE_URL=https://test-project.supabase.co');
      expect(envLocalCall[1]).toContain('SUPABASE_ANON_KEY=test-anon-key');
      expect(envLocalCall[1]).toContain('SUPABASE_SERVICE_ROLE_KEY=test-service-role-key');
      ...
      expect(envProdCall[1]).toContain('SUPABASE_URL=https://test-project.supabase.co');
    });
    ...
    it('should update existing environment file', async () => {
      ...
      expect(updatedContent).toContain('SUPABASE_URL=https://new-project.supabase.co');
    ...
    it('should handle values with special characters', async () => {
      ...
      expect(content).toContain('URL_WITH_PARAMS="https://example.com?param=value&other=123"');
      expect(content).toContain('JSON_VALUE="{\\"key\\": \\"value with spaces\\"}"');
    });
    ...
    it('should handle write errors gracefully', async () => {
      ...
      await expect(updateEnvironmentVariables({
        appPath: '/test/app',
        envVars
  ...
  describe('Database Schema Extraction', () => {
    it('should extract schema using pg_dump', () => {
      ...
      const result = execSync(`pg_dump "${postgresUrl}" --schema-only --no-owner --no-privileges`, { encoding: 'utf8' });
    ...
    it('should handle pg_dump errors', () => {
      ...
      expect(() => {
        execSync('pg_dump "invalid-url" --schema-only --no-owner --no-privileges', { encoding: 'utf8' });
  ...
  describe('Environment File Generation', () => {
    it('should generate correct production environment variables', () => {
      ...
      expect(expectedEnvVars['SUPABASE_URL']).toBe(productionConfig.supabaseUrl);
      expect(expectedEnvVars['POSTGRES_URL']).toContain(productionConfig.productionProjectRef);
      expect(expectedEnvVars['POSTGRES_URL']).toContain(productionConfig.dbPassword);
      expect(expectedEnvVars['NEXT_PUBLIC_SUPABASE_URL']).toBe(productionConfig.supabaseUrl);
    });
  ...
  describe('Error Handling', () => {
    it('should provide clear error messages for common issues', () => {
      const errors = {
        'Local Supabase not running': 'Local Supabase is not running. Please start it first.',
        'Invalid project reference': 'Project reference must be a valid Supabase project ID',
        'Schema extraction failed': 'Failed to extract database schema',
        'Environment file update failed': 'Failed to update environment variables'
      };
