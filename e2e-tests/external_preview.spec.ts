...
testSkipIfWindows("external preview window button appears", async ({ po }) => {
  ...
  await expect(po.page.locator('text=Open preview in external window')).toBeVisible();
});

testSkipIfWindows("external preview window button is disabled when no app URL", async ({ po }) => {
  await po.setUp();
...
testSkipIfWindows("external preview window button positioning", async ({ po }) => {
  await po.setUp();
  await po.sendPrompt("tc=basic");
  await po.clickTogglePreviewPanel();
  ...
  const externalPreviewButton = po.page.getByTestId("preview-open-external-window-button");
  const externalLinkButton = po.page.getByTestId("preview-open-browser-button");
