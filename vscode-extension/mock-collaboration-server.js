...
io.on('connection', (socket) => {
  ...
  socket.on('chat:message', (data) => {
    console.log(`Chat message in session ${data.sessionId}: ${data.message.message}`);
    socket.to(data.sessionId).emit('chat:message', data);
  });

  // Inline comments
  socket.on('inline:comment:add', (data) => {
    console.log(`Comment added in session ${data.sessionId} on line ${data.comment.line}`);
    socket.to(data.sessionId).emit('inline:comment:add', data);
  });

  socket.on('inline:comment:resolve', (data) => {
    console.log(`Comment resolved in session ${data.sessionId}: ${data.commentId}`);
