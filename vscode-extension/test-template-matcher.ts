...
} from "./src/templateMatcher";

console.log("Testing Template Matcher...\n");

// Test 1: E-commerce matching
console.log("Test 1: E-commerce keywords");
const ecomResults = matchTemplates(
  "I want to build an e-commerce store with payments",
);
console.log(`  Found ${ecomResults.length} matches`);
console.log(
  `  Best match: ${ecomResults[0]?.title || "None"} (${ecomResults[0]?.id})`,
);
...
console.log(
  `  Best match: ${authResults[0]?.title || "None"} (${authResults[0]?.id})`,
);
...
console.log(
  `  Best match: ${saasResults[0]?.title || "None"} (${saasResults[0]?.id})`,
);
...
console.log(
  `  Default template: ${defaultTemplate.title} (${defaultTemplate.id})\n`,
);
