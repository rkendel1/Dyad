...
export class DecoratorManager {
    ...
    private selectionDecorations: Map<string, vscode.TextEditorDecorationType> = new Map();
    private commentDecorations: vscode.TextEditorDecorationType;
    private disposables: vscode.Disposable[] = [];
    private userPositions: Map<string, { cursor?: vscode.Position; selection?: vscode.Range }> = new Map();
    ...
    private handleSelectionChange(data: Record<string, unknown>): void {
        const userId = data.userId as string;
        const selection = data.selection as { start: { line: number; character: number }; end: { line: number; character: number } };
        const user = data.user as CollaborationUser;
        const currentUser = this.collaborationService.getCurrentUser();
     ...
     */
    private updateCursorDecoration(userId: string, user: CollaborationUser): void {
     ...
     */
    private updateSelectionDecoration(userId: string, user: CollaborationUser): void {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            return;
        }
