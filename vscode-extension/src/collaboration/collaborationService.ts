...
export class CollaborationService {
  ...
  async startSession(
    ...
    try {
      this.outputChannel.appendLine(
        `Starting collaboration session for app: ${appName}`,
      );
      ...
      this.outputChannel.appendLine(
        `Collaboration session started: ${sessionId}`,
      );
      return this.currentSession;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.outputChannel.appendLine(
        `Failed to start collaboration session: ${message}`,
      );
  ...
  async joinSession(
    ...
    try {
      this.outputChannel.appendLine(
        `Joining collaboration session: ${sessionId}`,
      );
      ...
      return new Promise((resolve, reject) => {
        ...
        this.socket.emit(
          "session:join",
          { sessionId, user: this.currentUser },
          (response: {
            success: boolean;
            ...
            if (response.success && response.session) {
              ...
              this.outputChannel.appendLine(
                `Joined collaboration session: ${sessionId}`,
              );
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.outputChannel.appendLine(
        `Failed to join collaboration session: ${message}`,
      );
  ...
  async leaveSession(): Promise<void> {
    ...
    try {
      this.outputChannel.appendLine(
        `Leaving collaboration session: ${this.currentSession.id}`,
      );
  ...
  broadcastDocumentChange(
    change: Omit<DocumentChange, "sessionId" | "userId" | "timestamp">,
  ): void {
    if (!this.currentSession || !this.currentUser || !this.socket) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.socket.on("connect_error", (error) => {
          ...
          reject(
            new Error(
              `Failed to connect to collaboration server: ${error.message}`,
            ),
          );
        });
  ...
  private setupEventHandlers(): void {
    ...
    this.socket.on(
      CollaborationEventType.USER_CURSOR_MOVE,
      (data: Record<string, unknown>) => {
    ...
    this.socket.on(
      CollaborationEventType.USER_SELECTION_CHANGE,
      (data: Record<string, unknown>) => {
    ...
    // Document events
    this.socket.on(
      CollaborationEventType.DOCUMENT_CHANGE,
      (data: Record<string, unknown>) => {
    ...
    // Comment events
    this.socket.on(
      CollaborationEventType.INLINE_COMMENT_ADD,
      (data: Record<string, unknown>) => {
    ...
    this.socket.on(
      CollaborationEventType.INLINE_COMMENT_RESOLVE,
      (data: Record<string, unknown>) => {
    ...
    // Version events
    this.socket.on(
      CollaborationEventType.VERSION_SNAPSHOT,
      (data: Record<string, unknown>) => {
