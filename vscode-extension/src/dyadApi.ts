...
export class DyadApi {
    ...
    this.client.interceptors.response.use(
      ...
      (error: AxiosError) => {
        ...
        if (error.code === "ETIMEDOUT") {
          ...
          throw new Error(
            "Connection to Dyad Desktop timed out. Please check if Dyad Desktop is responding.",
          );
        }
        throw error;
      },
  ...
  async checkHealth(): Promise<boolean> {
    ...
    if (
      ...
      Date.now() - this.healthCheckCache.timestamp < this.HEALTH_CHECK_TTL
    ) {
    ...
    try {
      ...
      console.log(
        "Dyad Desktop API health check failed:",
  ...
  async getChats(appId: number): Promise<DyadChat[]> {
    try {
      const response = await this.client.get<DyadChat[]>(
  ...
  async sendMessage(
    chatId: number,
    content: string,
  ): Promise<DyadMessage | null> {
    try {
      const response = await this.client.post<DyadMessage>(
  ...
  async getMessages(chatId: number): Promise<DyadMessage[]> {
    try {
      const response = await this.client.get<DyadMessage[]>(
        `/api/chats/${chatId}/messages`,
      );
      return response.data;
    } catch (error) {
  ...
  async getAppStatus(appId: number): Promise<{ isRunning: boolean } | null> {
    try {
      const response = await this.client.get<{ isRunning: boolean }>(
  ...
  async setupLocalSupabase(
    params: SupabaseSetupParams,
  ): Promise<{ success: boolean; message?: string }> {
    try {
      const response = await this.client.post<{
        success: boolean;
        message?: string;
      }>("/api/supabase/setup-local", params);
      return response.data;
    } catch (error) {
      if (error instanceof Error) {
  ...
  async promoteToProduction(
    params: SupabasePromotionParams,
  ): Promise<{ success: boolean; message?: string }> {
    try {
      const response = await this.client.post<{
        success: boolean;
        message?: string;
      }>("/api/supabase/promote-to-production", params);
      return response.data;
    } catch (error) {
      if (error instanceof Error) {
