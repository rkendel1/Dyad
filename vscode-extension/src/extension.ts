...
export function activate(context: vscode.ExtensionContext) {
  ...
  const sidebarProvider = new DyadSidebarProvider(
  ...
  const collaborationSidebarProvider = new CollaborationSidebarProvider(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("dyad.runApp", async () => {
      try {
        ...
        if (apps.length === 0) {
          vscode.window.showInformationMessage(
            'No apps available to run. Create one first using Dyad Desktop or the "Create New App" command.',
          );
          return;
        }
        ...
        if (selectedApp) {
          try {
            outputChannel.appendLine(
              `Running app: ${selectedApp.label} (ID: ${selectedApp.id})`,
            );
            await dyadCli.runApp(selectedApp.id);
            vscode.window.showInformationMessage(
              `App "${selectedApp.label}" is running`,
            );
            outputChannel.appendLine(
              `App "${selectedApp.label}" started successfully`,
            );
            sidebarProvider.refresh();
          } catch (error) {
            const message =
    ...
    vscode.commands.registerCommand("dyad.stopApp", async () => {
      try {
        ...
        if (selectedApp) {
          try {
            outputChannel.appendLine(
              `Stopping app: ${selectedApp.label} (ID: ${selectedApp.id})`,
            );
            await dyadCli.stopApp(selectedApp.id);
            vscode.window.showInformationMessage(
              `App "${selectedApp.label}" stopped`,
            );
            outputChannel.appendLine(
              `App "${selectedApp.label}" stopped successfully`,
            );
            sidebarProvider.refresh();
          } catch (error) {
            const message =
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("dyad.checkHealth", async () => {
      ...
      if (isHealthy) {
        vscode.window.showInformationMessage(
          "✓ Successfully connected to Dyad Desktop",
        );
    ...
    vscode.commands.registerCommand("dyad.createAppWithTemplate", async () => {
      ...
      try {
        ...
        const selectedTemplate = await vscode.window.showQuickPick(
          ...
          {
            placeHolder:
              "Select a template for your app (auto-suggested based on your description)",
        ...
        outputChannel.appendLine(
          `Selected template: ${selectedTemplate.label} (${selectedTemplate.templateId})`,
        );
        ...
        outputChannel.appendLine(
          `Creating app: ${appName} with template: ${selectedTemplate.templateId}`,
        );
        const app = await dyadApi.createAppWithTemplate(
          appName,
          selectedTemplate.templateId || "react",
        );

        if (app) {
          vscode.window.showInformationMessage(
            `App "${appName}" created successfully with ${selectedTemplate.label}!`,
          );
          outputChannel.appendLine(`App "${appName}" created successfully`);
          sidebarProvider.refresh();
        } else {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("dyad.setupLocalSupabase", async () => {
      try {
        ...
        outputChannel.appendLine(
          `Setting up local Supabase for app: ${selectedApp.label} (ID: ${selectedApp.id})`,
        );

        const result = await dyadApi.setupLocalSupabase({
          appId: selectedApp.id,
        });

        if (result.success) {
          vscode.window.showInformationMessage(
            `Local Supabase setup successfully for "${selectedApp.label}"!`,
          );
          outputChannel.appendLine(
            `Local Supabase setup completed for app: ${selectedApp.label}`,
          );
        } else {
        ...
        vscode.window.showErrorMessage(
          `Failed to setup local Supabase: ${message}`,
        );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("dyad.promoteToProduction", async () => {
      try {
        ...
        outputChannel.appendLine(
          `Promoting app: ${selectedApp.label} (ID: ${selectedApp.id}) to production`,
        );
        ...
        if (result.success) {
          vscode.window.showInformationMessage(
            `Successfully promoted "${selectedApp.label}" to production Supabase!`,
          );
          outputChannel.appendLine(
            `Production promotion completed for app: ${selectedApp.label}`,
          );
        } else {
        ...
        vscode.window.showErrorMessage(
          `Failed to promote to production: ${message}`,
        );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("dyad.startCollaboration", async () => {
      try {
        ...
        outputChannel.appendLine(
          `Starting collaboration session for app: ${selectedApp.label}`,
        );
        ...
        outputChannel.appendLine(
          `Collaboration session created: ${session.id}`,
        );
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        outputChannel.appendLine(`Error starting collaboration: ${message}`);
        vscode.window.showErrorMessage(
          `Failed to start collaboration: ${message}`,
        );
      }
    }),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("dyad.joinCollaboration", async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to join collaboration: ${message}`,
        );
      }
    }),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("dyad.leaveCollaboration", async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to leave collaboration: ${message}`,
        );
      }
    }),
  );
...
async function checkDyadConnection() {
  try {
    ...
    if (!isHealthy) {
      outputChannel.appendLine(
        "Warning: Cannot connect to Dyad Desktop. Some features may not work.",
      );
...
function showDyadDesktopRequiredMessage() {
  const message =
    "This feature requires Dyad Desktop to be running. Please launch Dyad Desktop and try again.";
  vscode.window
    .showInformationMessage(message, "Open Dyad Website", "Check Connection")
    .then((action) => {
