...
export class DyadCli {
  ...
  async checkCliAvailability(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        "Dyad CLI not found in PATH. This is expected as Dyad is a desktop application.",
      );
  ...
  async createApp(appName: string): Promise<string> {
    ...
    if (!available) {
      throw new Error(
        "Dyad CLI is not available. Please use the Dyad Desktop application to create apps.",
      );
    }

    try {
      const { stdout, stderr } = await execAsync(
        `${this.dyadPath} create ${appName}`,
        { timeout: 30000 },
      );
  ...
  async runApp(appId: number): Promise<string> {
    ...
    if (!available) {
      throw new Error(
        "Dyad CLI is not available. Please use the Dyad Desktop application to run apps.",
      );
    }

    try {
      const { stdout, stderr } = await execAsync(
        `${this.dyadPath} run ${appId}`,
        { timeout: 30000 },
      );
      if (stderr && !stderr.includes("warning")) {
        console.error("Dyad CLI stderr:", stderr);
  ...
  async stopApp(appId: number): Promise<string> {
    ...
    if (!available) {
      throw new Error(
        "Dyad CLI is not available. Please use the Dyad Desktop application to stop apps.",
      );
    }

    try {
      const { stdout, stderr } = await execAsync(
        `${this.dyadPath} stop ${appId}`,
        { timeout: 30000 },
      );
      if (stderr && !stderr.includes("warning")) {
        console.error("Dyad CLI stderr:", stderr);
  ...
  async openConsole(): Promise<string> {
    ...
    if (!available) {
      throw new Error(
        "Dyad CLI is not available. Please use the Dyad Desktop application.",
      );
    }
