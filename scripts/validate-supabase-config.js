...
testAppIds.forEach((appId) => {
  ...
  console.log(
    `  ${appId.toString().padEnd(4)} | ${config.postgresPort.toString().padEnd(10)} | ${config.apiPort.toString().padEnd(10)} | ${config.dashboardPort.toString().padEnd(9)} | ${projectName}`,
  );
});
...
for (let i = 0; i < testAppIds.length; i++) {
  for (let j = i + 1; j < testAppIds.length; j++) {
    ...
    if (range1.max >= range2.min && range1.min <= range2.max) {
      console.log(
        `❌ FAIL: Port ranges overlap between app ${testAppIds[i]} and app ${testAppIds[j]}`,
      );
      overlapsFound = true;
    }
  }
...
testAppIds.forEach((appId) => {
  ...
  if (!/^dyad-supabase-\d+$/.test(projectName)) {
    console.log(
      `❌ FAIL: Invalid project name format for app ${appId}: ${projectName}`,
    );
  }
});
