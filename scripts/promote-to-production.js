...
async function promptForProjectDetails() {
  ...
  const useExisting = await question('Do you want to use an existing Supabase project? (y/N)');
  
  if (useExisting.toLowerCase() === 'y' || useExisting.toLowerCase() === 'yes') {
    const projectRef = await question('Enter your existing project reference ID: ');
    ...
    const projectName = await question('Enter a name for your new production project: ');
    const organizationId = await question('Enter your Supabase organization ID: ');
    const region = await question('Enter your preferred region (default: us-east-1): ') || 'us-east-1';
...
async function createProductionProject(projectDetails) {
  ...
  await question('Press Enter when you have created the project and have the credentials ready...');
...
async function migrateSchema(productionConfig, schema) {
  ...
  try {
    ...
    log('Schema written to temporary file. You can now apply it to your production database.');
    ...
    log('');
    log('OR use psql:');
    log(`psql "postgresql://postgres:${productionConfig.dbPassword}@db.${productionConfig.projectRef}.supabase.co:5432/postgres" -f ${tempSchemaFile}`);
    
    const confirmed = await question('Have you successfully applied the schema? (y/N)');
    if (confirmed.toLowerCase() !== 'y' && confirmed.toLowerCase() !== 'yes') {
      throw new Error('Schema migration was not confirmed. Please apply the schema before continuing.');
    }
...
async function migrateFunctions(_productionConfig) {
  ...
  log(`Found ${functions.length} function(s) to migrate: ${functions.join(', ')}`);
...
async function optionalDataMigration() {
  const shouldMigrate = await question('Do you want to migrate data from local to production? (y/N)');
  ...
  if (shouldMigrate.toLowerCase() === 'y' || shouldMigrate.toLowerCase() === 'yes') {
    ...
    log(`pg_dump "${LOCAL_SUPABASE_CONFIG.postgresUrl}" --data-only --no-owner --no-privileges > data.sql`);
    log('Then apply to production using the SQL editor or psql');
    log('');
    
    await question('Press Enter when you have completed data migration (or skip if not needed)...');
  }
}
...
async function performPostMigrationValidation(_productionConfig) {
  ...
  const validationPassed = await question('Have you verified all items above? (y/N)');
  
  if (validationPassed.toLowerCase() === 'y' || validationPassed.toLowerCase() === 'yes') {
...
async function showSummary(productionConfig, appPath) {
  ...
  log(`Dashboard: https://supabase.com/dashboard/project/${productionConfig.projectRef}`);
  log('');
  log('Updated Files:');
  ...
  log('- Keep your service role key secure and never expose it in client-side code');
...
async function main() {
  try {
    ...
    if (!checkLocalSupabaseRunning()) {
      throw new Error('Local Supabase is not running. Please start it first using: npm run supabase:start');
    }
    ...
    log('This process will update your environment files and promote your local development to production.');
    ...
    const confirmed = await question('Do you want to continue with the production promotion? (y/N)');
    if (confirmed.toLowerCase() !== 'y' && confirmed.toLowerCase() !== 'yes') {
    ...
    const validationPassed = await performPostMigrationValidation(productionConfig);
    
    if (validationPassed) {
      // Show summary
      await showSummary(productionConfig, appPath);
    } else {
      log('Production promotion completed with warnings. Please review and validate manually.', 'warning');
    }
