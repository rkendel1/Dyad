...
async function runTests() {
  ...
  if (appsResult?.data?.apps?.length > 0) {
    ...
    await testEndpoint(`GET /api/apps/${app.id}/settings`, `/api/apps/${app.id}/settings`);
    ...
    log('   No apps found. Create an app in Dyad Desktop first.', colors.yellow);
  }
  
  // Test 5: Error Handling
  log('\n5. Error Handling', colors.yellow);
  const notFound = await testEndpoint('GET /api/nonexistent', '/api/nonexistent');
  if (notFound?.success === false) {
    logSuccess('404 error handling works correctly');
  }
  
  const invalidId = await testEndpoint('GET /api/apps/999999', '/api/apps/999999');
